...
IMAGE_BACKGROUND = opt.IMAGE_BACKGROUND
CSS_FILE = opt.CSS_FILE
XTTS_REPO = opt.XTTS_REPO
# Streamlit UI
...
if base64_image:
    page_bg_img = load_css_with_image(CSS_FILE, IMAGE_BACKGROUND)
...
@st.cache_resource
def load_tts_model():
    config_path = os.path.join(XTTS_REPO, "config.json")
    speaker_file_path = os.path.join(XTTS_REPO, "speakers_xtts.pth")
    checkpoint_dir = XTTS_REPO
